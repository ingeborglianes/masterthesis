...
public class LS_operatorsFeasible {
    ...
    public LS_operatorsFeasible(int[][] OperationsForVessel, int[] vesseltypes, int[][][][] SailingTimes,
 ...
 */
        ...
        updateSimultaneous(simOpRoutes, vessel, simultaneousOp,
                ...
                precedenceOfOperations, vesselroutes, pos1, pos2, "onerelocate",EarliestStartingTimeForVessel);
 ...
 */
        ...
        updateSimultaneous(simOpRoutes, vessel2, simultaneousOp,
                precedenceOverRoutes, precedenceOfRoutes, TimeVesselUseOnOperation, startNodes, SailingTimes, precedenceOverOperations,
                precedenceOfOperations, vesselroutes, pos1, pos2, "noUpdate",EarliestStartingTimeForVessel);
    ...
    public Boolean one_exchange(int vessel, int cur_pos, int new_pos, int[] startnodes) {
        System.out.println("One exchange method run");
        /*
        for (int i=0;i<vesselroutes.size();i++){
            System.out.println("VESSELINDEX "+i);
            if (vesselroutes.get(i)!=null) {
                for (int t=0;t<vesselroutes.get(i).size();t++) {
                    System.out.println("Operation number: "+vesselroutes.get(i).get(t).getID() + " Earliest start time: "+
                            vesselroutes.get(i).get(t).getEarliestTime()+ " Latest Start time: "+ vesselroutes.get(i).get(t).getLatestTime());
                }
            }
        }
        System.out.println(" ");
        ...
        System.out.println("Is solution feasible? "+checkSolution(vesselroutes,startNodes,TimeVesselUseOnOperation,precedenceOverOperations,
                simultaneousOp));

         */
        if (cur_pos == new_pos) {
            System.out.println("cur_pos=new_pos");
            return false;
        }
        ...
        System.out.println("Operation position 1 "+toMove1.getID());
        System.out.println("Operation position 2 "+toMove2.getID());
 ...
 */
        ...
        updateSimultaneous(simOpRoutes, vessel, simultaneousOp,
                precedenceOverRoutes, precedenceOfRoutes, TimeVesselUseOnOperation, startNodes, SailingTimes, precedenceOverOperations,
                precedenceOfOperations, vesselroutes, pos1, pos2, "oneExchange",EarliestStartingTimeForVessel);
 ...
 */
        ...
        System.out.println("Remove operation 1 "+toMoveOriginal1.getID()+" vessel "+vessel1+" index "+ pos1);
        System.out.println("Remove operation 2 "+toMoveOriginal2.getID()+" vessel "+vessel2+" index "+ pos2);
        ...
        System.out.println("Vessel 1 "+vessel1 +" updated earliest and latest");
        System.out.println("Vessel 2 "+vessel2+" updated earliest and latest");
...
*/
        ...
        updateSimultaneous(simOpRoutes, vessel2, simultaneousOp,
                precedenceOverRoutes, precedenceOfRoutes, TimeVesselUseOnOperation, startNodes, SailingTimes, precedenceOverOperations,
                precedenceOfOperations, vesselroutes, pos1, pos2, "twoExchange",EarliestStartingTimeForVessel);
    ...
    public Boolean insert(OperationInRoute unrouted_operation, int vessel, int pos1, int[] startnodes) {
        ...
        updateSimultaneous(simOpRoutes, vessel, simultaneousOp,
                precedenceOverRoutes, precedenceOfRoutes, TimeVesselUseOnOperation, startNodes, SailingTimes, precedenceOverOperations,
                precedenceOfOperations, vesselroutes, pos1, 1000, "insert",EarliestStartingTimeForVessel);
    ...
    public static void updateLatest(int latest, int indexInRoute, int routeIndex,
        ...
        for (int k = indexInRoute - 1; k > -1; k--) {
            ...
            if (!method.equals("oneexchange") && newTime == objectK.getLatestTime()) {
                System.out.println("break on ID "+objectK.getID());
                break;
            }
    ...
    public static boolean updateSimultaneous(List<Map<Integer, ConnectedValues>> simOpRoutes, int routeIndex,
                                             ...
                                             int pos1, int pos2, String function, int[] EarliestStartingTimeForVessel) {
        ...
        if (simOpRoutes.get(routeIndex) != null) {
            for (ConnectedValues sValues : simOpRoutes.get(routeIndex).values()) {
                ...
                if (sValues.getConnectedOperationObject() != null) {
                    ...
                    int latestPO;
                    if (simOp.getIndex() == 0) {
                        earliestPO = SailingTimes[simOp.getRoute()][EarliestStartingTimeForVessel[simOp.getRoute()]][startNodes[simOp.getRoute()] - 1][simOp.getOperationObject().getID() - 1];
                    } else {
                    ...
                    if (simOp.getIndex() == vesselroutes.get(simOp.getRoute()).size() - 1) {
                        ...
                        System.out.println("first op: "+sValues.getOperationObject().getID()+ " latest: "+sValues.getOperationObject().getLatestTime());
                    ...
                    System.out.println("latest PO "+latestPO);
                    System.out.println("cur_latestTemp "+cur_latestTemp);
                    ...
                    if (latestTemp < cur_latestTemp) {
                        ...
                        feasible = ConstructionHeuristic.updateLatest(cur_latestTemp, sValues.getIndex(), routeIndex, TimeVesselUseOnOperation, startNodes, SailingTimes, vesselroutes,"local",simultaneousOp,
                                precedenceOfOperations,precedenceOverOperations);
                        if(!feasible){
                            return feasible;
                        }
        ...
        for (int route : routesToUpdate) {
            if (vesselroutes.get(route) != null && vesselroutes.get(route).size() != 0) {
                ...
                feasible = updateSimultaneous(simOpRoutes, route, simultaneousOp, precedenceOverRoutes, precedenceOfRoutes, TimeVesselUseOnOperation, startNodes, SailingTimes, precedenceOverOperations, precedenceOfOperations,
                        vesselroutes, 0, 0, "conRoutes",EarliestStartingTimeForVessel);
                if(!feasible){
                    return feasible;
                }
            }
                ...
                if (n == -1) {
                    ...
                    if (earliestTemp <= 60) {
                        if (earliestTemp - 1 < nTimePeriods) {
                            ...
                            System.out.println("Optime "+o +": "+opTime);
                            System.out.println("Sailing time o to next: "+sailingTimeOToNext);
                            System.out.println("Latest time next "+vesselroutes.get(v).get(n + 1).getLatestTime());
                            ...
                            if (earliestTemp <= latestTemp && pPlacementFeasible) {
                                ...
                                int deltaOperationGainLastOperation = 0;
                                System.out.println("Insert op "+o);
                                System.out.println("insert index "+(n+1));
                                System.out.println("insert vessel "+v);
                                System.out.println("Earliest temp "+earliestTemp);
                                System.out.println("Latest temp "+latestTemp);
                                System.out.println(earliestTimeLastOperationInRoute + changedTime - 1);
                                /*
                                for (int i=0;i<vesselroutes.size();i++){
                                    System.out.println("VESSELINDEX "+i);
                                    if (vesselroutes.get(i)!=null) {
    ...
    public static CopyValues retainOldSolution(List<List<OperationInRoute>> vesselroutes, Map<Integer,ConsolidatedValues> consolidatedOperations,
        ...
        for(Map.Entry<Integer, PrecedenceValues> entry : precedenceOfOperationsCopy.entrySet()){
            ...
            if(pvOf.getConnectedOperationObject()==null){
                ...
                System.out.println(connectedPOverTaskID);
    ...
    public void oneExchangeAll(){
        ...
        boolean done = false;
        while(!done) {
            System.out.println("new loop in while");
            for(int vessel=0; vessel<vesselroutes.size();vessel++) {
                if (vesselroutes.get(vessel) != null) {
                    for (int position = 0; position < vesselroutes.get(vessel).size(); position++) {
                        for (int task = 0; task < vesselroutes.get(vessel).size(); task++) {
                            ...
                            if (changePerformed || lastChange == null) {
                                ...
                                if(changePerformed){
                                    System.out.println("Break from one exchange all");
                                    done=true;
                                }
                            } else {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        LS_operatorsFeasible LSO = new LS_operatorsFeasible(dg.getOperationsForVessel(),vesseltypes,dg.getSailingTimes(),dg.getTimeVesselUseOnOperation(),
